...
Public Sub CommandPass(ByVal srcPerformWhat As String)
    ...
    Select Case srcPerformWhat
        Case "New"
            frmInvoiceAE.State = adStateAddMode
            frmInvoiceAE.show vbModal
        Case "Edit"
            If lvList.ListItems.Count > 0 Then
                If isRecordExist("tbl_AR_Invoice", "PK", CLng(LeftSplitUF(lvList.SelectedItem.Tag))) = False Then
        ...
        Case "Delete"
            If lvList.ListItems.Count > 0 Then
                If isRecordExist("tbl_AR_Invoice", "PK", CLng(LeftSplitUF(lvList.SelectedItem.Tag))) = False Then
                    ...
                    Exit Sub
                Else
                    If getValueAt("SELECT PK,Lock FROM tbl_AR_Invoice WHERE PK=" & CLng(LeftSplitUF(lvList.SelectedItem.Tag)) & "", "Lock") = "Y" Then
                        MsgBox "The selected invoice record cannot be remove or void because it is already been recorded in van remmitance.", vbExclamation
                        Exit Sub
                    End If
                    ...
                    If ANS = vbYes Then
                        'Remove
                        DelRecwSQL "tbl_AR_Invoice", "PK", "", True, CLng(LeftSplitUF(lvList.SelectedItem.Tag))
                        'Refresh the records
                        RefreshRecords
                        MAIN.UpdateInfoMsg
...
Private Sub Form_Load()
    ...
    With SQLParser
        .Fields = "InvoiceNo,Date,SoldTo,Address,VanName,Salesman,ChargeAccount,PaymentType,BilledIn,Discount,TotalAmount,DownPayment,AmountPaid,Balance,Remarks,Paid,SoldToPK,VanFK,SalesmanFK,LastLoadingFK,PK"
        .Tables = "qry_AR_Invoice"
        .SortOrder = "PK DESC"
