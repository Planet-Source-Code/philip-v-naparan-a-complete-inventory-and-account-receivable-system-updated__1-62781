...
Begin VB.Form frmVanInventory 
   ...
   End
End
Attribute VB_Name = "frmVanInventory"
...
Public Sub CommandPass(ByVal srcPerformWhat As String)
    ...
    Select Case srcPerformWhat
        Case "New"
            frmVanInventoryAE.State = adStateAddMode
            frmVanInventoryAE.show vbModal
        Case "Edit"
            If lvList.ListItems.Count > 0 Then
                If isRecordExist("tbl_IC_VanInv", "PK", CLng(LeftSplitUF(lvList.SelectedItem.Tag))) = False Then
                    MsgBox "This record has been removed by other user.Click 'OK' button to refresh the records.", vbExclamation
                    RefreshRecords
                    Exit Sub
                Else
                    With frmVanInventoryAE
        ...
        Case "Delete"
            If lvList.ListItems.Count > 0 Then
                If isRecordExist("tbl_IC_VanInv", "PK", CLng(LeftSplitUF(lvList.SelectedItem.Tag))) = False Then
                    ...
                    Exit Sub
                Else
                    If getValueAt("SELECT PK,Lock FROM tbl_IC_VanInv WHERE PK=" & CLng(LeftSplitUF(lvList.SelectedItem.Tag)) & "", "Lock") = "Y" Then
