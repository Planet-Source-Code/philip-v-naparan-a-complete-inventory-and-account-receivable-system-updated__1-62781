...
Public Sub CommandPass(ByVal srcPerformWhat As String)
    ...
    Select Case srcPerformWhat
        Case "New"
            frmStockReceiveAE.State = adStateAddMode
            frmStockReceiveAE.show vbModal
        Case "Edit"
            If lvList.ListItems.Count > 0 Then
                If isRecordExist("tbl_IC_StockReceive", "PK", CLng(LeftSplitUF(lvList.SelectedItem.Tag))) = False Then
                    MsgBox "This record has been removed by other user.Click 'OK' button to refresh the records.", vbExclamation
                    RefreshRecords
                    Exit Sub
                Else
                    With frmStockReceiveAE
        ...
        Case "Delete"
                ...
                If lvList.ListItems.Count > 0 Then
                    ...
                    Else
                        ...
                        Dim ANS As Integer
                        ANS = MsgBox("Are you sure you want to delete the selected record?" & vbCrLf & vbCrLf & "WARNING: You cannot undo this operation.This will permanently remove the record.", vbCritical + vbYesNo, "Confirm Record")
                        ...
                        If ANS = vbYes Then
                            ...
                            Dim ProdPK As Long 'Product PK
                            
                            CN.BeginTrans

                            ProdPK = toNumber(getValueAt("SELECT PK,ProductFK FROM tbl_IC_StockReceive WHERE PK=" & CLng(LeftSplitUF(lvList.SelectedItem.Tag)), "ProductFK"))
                            ...
                            ChangeValue CN, "tbl_IC_Products", "Cases", tC - toNumber(lvList.SelectedItem.ListSubItems(3).Text), True, "WHERE PK=" & ProdPK
                            ChangeValue CN, "tbl_IC_Products", "Boxes", tB - toNumber(lvList.SelectedItem.ListSubItems(4).Text), True, "WHERE PK=" & ProdPK
                            ChangeValue CN, "tbl_IC_Products", "Pieces", tP - toNumber(lvList.SelectedItem.ListSubItems(5).Text), True, "WHERE PK=" & ProdPK
                            
                            'Remove the main
                            DelRecwSQL "tbl_IC_StockReceive", "PK", "", True, CLng(LeftSplitUF(lvList.SelectedItem.Tag))
        ...
        Case "Refresh"
            RefreshRecords
        Case "Print"
            frmStockInPrintOp.show vbModal
        Case "Close"
            Unload Me
    End Select
    Exit Sub
...
Private Sub Form_Load()
    ...
    With SQLParser
        .Fields = "*"
        .Tables = "qry_IC_StockReceive"
        .SortOrder = "DateReceive DESC,ProductCode ASC"
        
        .SaveStatement
    End With
    
    rsStockR.CursorLocation = adUseClient
    rsStockR.Open SQLParser.SQLStatement, CN, adOpenStatic, adLockReadOnly
...
Private Sub FillList(ByVal whichPage As Long)
    ...
    Me.Enabled = False
    Call pageFillListView(lvList, rsStockR, RecordPage.PageStart, RecordPage.PageEnd, 16, 2, False, True, , , , "PK")
    Me.Enabled = True
